...
Reference implementation: [ess-docker-compose](https://github.com/wlphi/ess-docker-compose) — the same stack as Docker Compose.
...
```
flake.nix              # inputs/outputs, the two nixosConfigurations + checks
modules/
  default.nix          # imports every module below
  options.nix          # nixmatrix.* options (domain, sso, bridges, externalProxy, …)
  synapse.nix          # Synapse homeserver + MSC3861 delegation to MAS
  mas.nix              # Matrix Authentication Service
  caddy.nix            # reverse proxy, TLS, all Matrix routing + well-known
  postgres.nix         # PostgreSQL + per-service users + runtime password set
  redis.nix
  element-web.nix      # Element Web + FluffyChat + admin panel (Ketesa)
  authelia.nix         # optional SSO / upstream OIDC (nixmatrix.sso.enable)
  livekit.nix          # Element Call backend (LiveKit + lk-jwt-service)
  monitoring.nix       # Prometheus + exporters + Grafana
  disk.nix             # disko layout for nixos-anywhere
  bridges/
    doublepuppet.nix   # double-puppet appservice registration
    telegram.nix       # each gated on nixmatrix.bridges.<net>.enable
    whatsapp.nix
    signal.nix
    discord.nix
secrets/
  secrets.yaml         # sops-encrypted (generated by scripts/bootstrap.sh)
hosts/
  matrix-server.nix    # top-level host config (the file you edit per deploy)
test/
  integration.nix      # NixOS VM test, wired in as a flake check
  check-nix.sh         # static config checks
  smoke-test.sh        # assertions against a running VM/host
examples/
  reverse-proxy/       # drop-in nginx + Apache configs for externalProxy mode
```

(Slack and IRC/Hookshot bridges were scoped in the original plan but aren't
implemented yet.)
...
inputs = {
  ...
  # MAS module — pinned to a specific rev for reproducibility (see flake.nix).
  nixos-matrix-modules.url = "github:D4ndellion/nixos-matrix-modules";
  nixos-anywhere.url = "github:numtide/nixos-anywhere";  # one-command VPS install
  disko.url = "github:nix-community/disko";              # declarative partitioning
};
```

See `flake.nix` for the exact pins. The flake exposes two
`nixosConfigurations` (`matrix-server` for production, `matrix-server-vm` for the
local test VM) and a `checks.<system>.integration` VM test.
...
- Reference Docker implementation: [ess-docker-compose](https://github.com/wlphi/ess-docker-compose)
